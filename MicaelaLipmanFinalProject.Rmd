...
LongForm <-read.csv("data/LongForm.csv")

##Merging Data
counties_sf <- get_urbn_map("counties", sf = TRUE) %>% 
  filter(state_abbv=="NY") 

AllCounties<-left_join(x=counties_sf, y=LongForm2, by="county_name", all.x=TRUE) 
...
loop <- animate( anim2, fps = 10,
nframes = 22,
end_pause = 3) + labs(subtitle = {"frame_time"})
...
1) Why can't I get the data to load from github? 

2) How should I show evidence of many trials? Should I narrow down to one data visualization to present and just show the code for the others? Should I show multiple drafts of broken code I worked through?

3) Why are some counties showing up as NA when I have data for these counties in the excel file? I tried to use the following code and the file is on github but when I read it into R it seems to only have metadata not data. 

  LongForm <-read.csv("https://github.com/GEO511-2021/2021_project   -MicaelaFLipman/blob/ea0119a7c180fabed5d48131db971a109c57de23/Lo   ngForm.csv")

4) How would you like the code submitted? I submitted the full code as text but is there another format? Should I let 

5) Why can't I get the subtitle to change as year changes in the animated maps? I've tried using the code below but it comes up with an error every time. I think the error might be that since I don't have data for every year I had to use transition_manual(Year) so R doesn't read Year as continuous???
